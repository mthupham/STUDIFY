...
2. Optionally, before starting, the User may apply a tag filter to select a specific subset of flashcards to study (refer to UC4 – Filter Flashcards by Tag). If filtered, only flashcards matching the selected tag(s) are included in the session deck.
...
5. The User flips the card to view the **Explanation** on the back side (refer to UC9 – Flip Card to View Explanation). This is a mandatory included use case.

6. After reviewing the Explanation, the User marks their study status for the current card (refer to UC10 – Mark Study Status). This is a mandatory included use case.
...
This alternative flow occurs when the User wants to study a specific tagged subset of flashcards. This delegates to the full specification in UC4 (Filter Flashcards by Tag).
...
This use case is extended by UC4 (Filter Flashcards by Tag) at the study session setup step (Step 2 of the Basic Flow), when the User chooses to restrict the study deck to a specific tag-filtered subset of flashcards.

### 6.2 Flip Card to View Explanation

This use case includes UC9 (Flip Card to View Explanation) as a mandatory step (Step 5 of the Basic Flow). For each flashcard, the User must have the ability to flip the card to see the Explanation on the back side.

### 6.3 Mark Study Status

This use case includes UC10 (Mark Study Status) as a mandatory step (Step 6 of the Basic Flow). After reviewing each card, the User must mark their self-assessed study status.
