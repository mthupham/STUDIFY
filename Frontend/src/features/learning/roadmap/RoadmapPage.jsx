...
export default function RoadmapPage() {
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      const levelIndex = Math.max(
        0,
        Math.min(centerViewIndex, roadmapData.views.length - 1),
      );
      ...
      try {
        ...
        if (!cancelled) {
          ...
          const assigned =
            ...
            (payload.metrics && payload.metrics.level
              ? payload.metrics.level.substring(0, 2)
              : "A1");
          setAssignedLevel(assigned);
          setCurrentLevelTitle(
            payload.levelTitle || levelLabels[assigned] || "Beginner",
          );
        }
      } catch (error) {
        if (!cancelled) {
  ...
  useEffect(() => {
    ...
    const levelIndex = roadmapData.levels.findIndex(
      (lvl) => lvl === assignedLevel,
    );
      ...
      <div style={styles.card}>
        <header style={styles.header}>
          <div>
            ...
            <p style={styles.sub}>
              Mastering English: From Beginner to Fluency
            </p>
          </div>
        ...
        <div
          className="roadmapScroll"
          ref={scrollRef}
          style={styles.scrollArea}
        >
          <div style={styles.track}>
            <svg
              width={roadmapData.views.length * SECTION_WIDTH}
              height="320"
              style={styles.pathSvg}
              viewBox={`0 0 ${roadmapData.views.length * SECTION_WIDTH} 320`}
            >
              <path
          ...
          >
            ...
            <div style={styles.metricCol}>
              ...
              <div style={styles.metricValue}>
                {roadmapData.metrics.lessons}
              </div>
            </div>
            <div style={styles.metricCol}>
              <div style={styles.metricLabel}>STREAK</div>
              <div style={styles.metricValue}>
                🔥 {roadmapData.metrics.streak}
              </div>
            </div>
          </button>
          <button
...
const styles = {
  ...
  card: {
    width: "100%",
    maxWidth: 1400, // hoặc 1280, 1440 tùy thiết kế
    margin: "0 auto",
    background: "#fff",
    borderRadius: 24,
    padding: 28,
