...
function IconCustomizationModal({
  ...
  // Cập nhật lại temp state mỗi khi modal mở
  React.useEffect(() => {
    if (isOpen) {
...
export default function CreateStudyGroupPage() {
  ...
  const [accessCode, setAccessCode] = useState("742-910");
  const [selectedIconId, setSelectedIconId] = useState("books");
  const [message, setMessage] = useState("");
  const [messageType, setMessageType] = useState<"success" | "error" | "">("");

  // State điều khiển mở Pop-up Modal
  const [isModalOpen, setIsModalOpen] = useState(false);

  // Hàm tạo ngẫu nhiên mã truy cập 6 chữ số
  const handleGenerateCode = () => {
    const random6Digits = Math.floor(
      100000 + Math.random() * 900000,
    ).toString();
    const formatted = `${random6Digits.slice(0, 3)}-${random6Digits.slice(3)}`;
        ...
        <div className="space-y-2">
          <button
            ...
            className="
                ...
                px-3 py-2 rounded-lg text-sky-700 text-sm font-semibold
                transition-all duration-200 hover:bg-sky-50 hover:text-sky-800
        ...
        >
          ...
          <div className="lg:col-span-2 bg-white rounded-2xl border border-slate-200 p-6 sm:p-8 shadow-sm space-y-8">
              ...
              <div className="p-6 bg-violet-50/70 border border-violet-100 rounded-2xl flex flex-col sm:flex-row items-center justify-between gap-6">
                <div className="space-y-1 text-center sm:text-left">
                  ...
                  <p className="text-xs text-gray-600 max-w-xs leading-relaxed">
                    Share this code with learners you want to invite to your
                    group.
                  </p>
                </div>

                <div className="flex flex-wrap items-center justify-center gap-3 w-full sm:w-auto">
                  <div className="h-12 px-6 bg-white border-2 border-sky-600/30 rounded-xl flex items-center justify-center text-sky-700 font-mono text-2xl font-bold tracking-widest shadow-sm">
                    {accessCode}
                  </div>
                  <button
                    type="button"
                    onClick={handleGenerateCode}
                    className="h-12 px-5 bg-sky-700 hover:bg-sky-800 text-white rounded-xl text-sm font-semibold shadow-sm transition-all flex items-center gap-2 shrink-0"
                  >
                    <svg
            ...
            <div className="pt-4 flex items-center justify-end gap-4">
              <button
                onClick={() => navigate("/study-groups")}
                type="button"
                className="px-6 py-3 rounded-xl border border-sky-700 text-sky-700 hover:bg-sky-50 text-sm font-semibold transition-colors"
              >
                Cancel
              </button>
              <button
                type="submit"
                className="px-8 py-3 rounded-xl bg-sky-700 hover:bg-sky-800 text-white text-sm font-semibold shadow-sm transition-colors"
              >
                Create Group
              </button>
            </div>
          ...
          <div className="space-y-6">
            <div className="bg-indigo-50/50 border border-slate-200 rounded-2xl p-6 sm:p-8 flex flex-col items-center text-center space-y-6">
              ...
              {/* AVATAR PREVIEW + NÚT CÂY BÚT SỬA ICON */}
              <div className="relative group">
                ...
                {/* Nút Cây bút (Pencil Button) mở Modal */}
                <button
                  type="button"
                  onClick={() => setIsModalOpen(true)}
